...
class General_Analysis(object):
    ...
    def createSteps(self,*args,**kwargs):
        ...
        if self.analysisType.startswith('Standard'):
            ...
            if self.analysisType.endswith('- Static'):
                ...
                self.mdb.models['Model-1'].StaticStep(name='Indent', previous='Initial',
                ...
                self.mdb.models['Model-1'].StaticStep(name='Remove', previous='Indent',
                ...
                if self.stepTime[1] > 0.0:
                    ...
                    self.mdb.models['Model-1'].ImplicitDynamicsStep(name='Indent', previous='Initial',
                    ...
                    self.mdb.models['Model-1'].ImplicitDynamicsStep(name='Dwell-Loaded', previous='Indent',
                        maxNumInc=10000, application=QUASI_STATIC, initialInc=self.stepTime[1]*1e-05, minInc=1e-15, timePeriod=self.stepTime[1],
                        nohaf=OFF, amplitude=RAMP, alpha=DEFAULT, initialConditions=OFF,
                        matrixStorage=UNSYMMETRIC, nlgeom=ON)

                    self.mdb.models['Model-1'].ImplicitDynamicsStep(name='Remove', previous='Dwell-Loaded',
                    ...
                    self.mdb.models['Model-1'].ImplicitDynamicsStep(name='Remove', previous='Indent',
                        maxNumInc=10000, application=QUASI_STATIC, initialInc=self.stepTime[2]*1e-05, minInc=1e-15, timePeriod=self.stepTime[2],
                        nohaf=OFF, amplitude=RAMP, alpha=DEFAULT, initialConditions=OFF,
                        matrixStorage=UNSYMMETRIC)
            ...
            if self.jobName.endswith('_pre'):
                ...
                self.mdb.models['Model-1'].FieldOutputRequest(name='Reaction_Force_Combo_A', createStepName='Indent', variables=('U', ), numIntervals=self.historyIntervals, timeMarks=ON, region=regionDef1, sectionPoints=DEFAULT, rebar=EXCLUDE)
                self.mdb.models['Model-1'].FieldOutputRequest(name='Reaction_Force_Combo_B', createStepName='Indent', variables=('RF', ), numIntervals=self.historyIntervals, timeMarks=ON, region=regionDef2, sectionPoints=DEFAULT, rebar=EXCLUDE)
                ...
                if self.indType == 'Rigid':
                    ...
                    self.mdb.models['Model-1'].FieldOutputRequest(name='F-Output-2', createStepName='Indent', variables=('LE', 'U', 'COORD'),
                ...
                if self.taMatPlast == 'GTN-pmp':
                    ...
                    self.mdb.models['Model-1'].FieldOutputRequest(name='Plastic_Zone', createStepName='Indent', variables=('PE', 'PEEQ', 'VVF','VVFG','VVFN', 'RD'),
                        numIntervals=self.fieldIntervals, timeMarks=ON, region=regionDef, sectionPoints=DEFAULT, rebar=EXCLUDE)

                else:

                    regionDef=self.mdb.models['Model-1'].rootAssembly.allInstances['Test_Article-1'].sets['Mat_Plastic_Set']
                    self.mdb.models['Model-1'].FieldOutputRequest(name='Plastic_Zone', createStepName='Indent', variables=('PE', 'PEEQ', 'RD'),
                        numIntervals=self.fieldIntervals, timeMarks=ON, region=regionDef, sectionPoints=DEFAULT, rebar=EXCLUDE)

                self.mdb.models['Model-1'].FieldOutputRequest(name='Contact_Results', createStepName='Indent', variables=('CSTRESS', 'CDISP', 'CFORCE'), numIntervals=self.fieldIntervals, timeMarks=ON)

                # regionDef=self.mdb.models['Model-1'].rootAssembly.allInstances['Test_Article-1'].sets['Set-1']
                # self.mdb.models['Model-1'].FieldOutputRequest(name='Elemental_Energies_Volumes', createStepName='Indent', variables=('ENER', 'ELEN', 'ELEDEN', 'EVOL', 'IVOL'),
                    # numIntervals=self.fieldIntervals, timeMarks=ON, region=regionDef, sectionPoints=DEFAULT, rebar=EXCLUDE)
            ...
            self.mdb.models['Model-1'].steps['Indent'].control.setValues(allowPropagation=OFF, resetDefaultValues=OFF, timeIncrementation=(4.0, 8.0, 9.0, 16.0, 10.0, 4.0, 12.0, 20.0, 6.0, 3.0, 50.0))

        elif self.analysisType.startswith('Explicit'):

            if self.stepTime[1] > 0.0:

                self.mdb.models['Model-1'].ExplicitDynamicsStep(name='Indent', previous='Initial', timePeriod=self.stepTime[0])
                self.mdb.models['Model-1'].ExplicitDynamicsStep(name='Dwell-Loaded', previous='Indent', timePeriod=self.stepTime[1])
                self.mdb.models['Model-1'].ExplicitDynamicsStep(name='Remove', previous='Dwell-Loaded', timePeriod=self.stepTime[2])

                self.mdb.models['Model-1'].steps['Indent'].setValues(linearBulkViscosity=self.bulkViscosity[0])
                self.mdb.models['Model-1'].steps['Dwell-Loaded'].setValues(linearBulkViscosity=self.bulkViscosity[1])
                self.mdb.models['Model-1'].steps['Remove'].setValues(linearBulkViscosity=self.bulkViscosity[2])

                self.mdb.models['Model-1'].steps['Indent'].setValues(timeIncrementationMethod=AUTOMATIC_EBE, scaleFactor=1.0, maxIncrement=None)
                ...
                self.mdb.models['Model-1'].steps['Remove'].setValues(timeIncrementationMethod=AUTOMATIC_EBE, scaleFactor=1.0, maxIncrement=None)

            else:

                self.mdb.models['Model-1'].ExplicitDynamicsStep(name='Indent', previous='Initial', timePeriod=self.stepTime[0])
                self.mdb.models['Model-1'].ExplicitDynamicsStep(name='Remove', previous='Indent', timePeriod=self.stepTime[2])
            ...
            if self.stepTime[3] > 0.0:
                ...
                self.mdb.models['Model-1'].ExplicitDynamicsStep(name='Dwell-Unloaded', previous='Remove', timePeriod=self.stepTime[3])
            ...
            if self.analysisType.endswith('Mass Scaling'):
                ...
                self.mdb.models['Model-1'].steps['Indent'].setValues(massScaling=((SEMI_AUTOMATIC, MODEL, THROUGHOUT_STEP, 0.0, 1.0, BELOW_MIN, 100, 0, 0.0, 0.0, 0, None), ))
            ...
            if self.jobName.endswith('_pre'):
                ...
                regionDef1=self.mdb.models['Model-1'].rootAssembly.sets['mSet-1']
                regionDef2=self.mdb.models['Model-1'].rootAssembly.allInstances['Test_Article-1'].sets['Base_Set']
                self.mdb.models['Model-1'].FieldOutputRequest(name='Reaction_Part_A', createStepName='Indent', variables=('U', 'RF'), numIntervals=self.historyIntervals, timeMarks=ON, region=regionDef1, sectionPoints=DEFAULT, rebar=EXCLUDE)
                self.mdb.models['Model-1'].FieldOutputRequest(name='Reaction_Part_B', createStepName='Indent', variables=('U', 'RF'), numIntervals=self.historyIntervals, timeMarks=ON, region=regionDef2, sectionPoints=DEFAULT, rebar=EXCLUDE)
                ...
                if self.indType == 'Rigid':
                    ...
                    regionDef=self.mdb.models['Model-1'].rootAssembly.allInstances['Indenter-1'].sets['Set-1']
                    self.mdb.models['Model-1'].FieldOutputRequest(name='F-Output-2', createStepName='Indent', variables=('LE', 'U', 'V', 'A', 'COORD'),
                        numIntervals=self.fieldIntervals, timeMarks=ON, region=regionDef, sectionPoints=DEFAULT, rebar=EXCLUDE)

                if self.taMatPlast == 'GTN-pmp':

                    regionDef=self.mdb.models['Model-1'].rootAssembly.allInstances['Test_Article-1'].sets['Mat_Plastic_Set']
                    self.mdb.models['Model-1'].FieldOutputRequest(name='Plastic_Zone', createStepName='Indent', variables=('PE', 'PEEQ', 'DENSITY', 'VVF','VVFG','VVFN'),
                        numIntervals=self.fieldIntervals, timeMarks=ON, region=regionDef, sectionPoints=DEFAULT, rebar=EXCLUDE)

                else:

                    regionDef=self.mdb.models['Model-1'].rootAssembly.allInstances['Test_Article-1'].sets['Mat_Plastic_Set']
                    self.mdb.models['Model-1'].FieldOutputRequest(name='Plastic_Zone', createStepName='Indent', variables=('PE', 'PEEQ', 'DENSITY'),
                        numIntervals=self.fieldIntervals, timeMarks=ON, region=regionDef, sectionPoints=DEFAULT, rebar=EXCLUDE)

                self.mdb.models['Model-1'].FieldOutputRequest(name='Contact_Results', createStepName='Indent', variables=('CSTRESS', 'CDISP', 'CFORCE', 'FSLIPR', 'FSLIP'), numIntervals=self.fieldIntervals, timeMarks=ON)

                # regionDef=self.mdb.models['Model-1'].rootAssembly.allInstances['Test_Article-1'].sets['Set-1']
                # self.mdb.models['Model-1'].FieldOutputRequest(name='Elemental_Energies_Volumes', createStepName='Indent', variables=('ENER', 'ELEN', 'ELEDEN', 'EDCDEN', 'EDT', 'EVOL'),
                    # numIntervals=self.fieldIntervals, timeMarks=ON, region=regionDef, sectionPoints=DEFAULT, rebar=EXCLUDE)
    ...
    def createRemeshing(self,*args,**kwargs):
        ...
        if self.analysisType.startswith('Explicit'):
            ...
            self.mdb.models['Model-1'].steps['Indent'].AdaptiveMeshDomain(region=region, controls='Adaptive_Mesh_Control')
            ...
            self.mdb.models['Model-1'].steps['Indent'].adaptiveMeshDomains['Indent'].setValues(frequency=self.remeshingParameters[0], meshSweeps=self.remeshingParameters[1])

            if self.taType.endswith('Pillar'):

                region=self.mdb.models['Model-1'].rootAssembly.instances['Test_Article-1'].sets['Remeshing_Set']
                self.mdb.models['Model-1'].steps['Remove'].AdaptiveMeshDomain(region=region, controls='Adaptive_Mesh_Control')

                self.mdb.models['Model-1'].adaptiveMeshControls['Adaptive_Mesh_Control'].setValues(volumetricSmoothingWeight=0.50, laplacianSmoothingWeight=0.50, equipotentialSmoothingWeight=0.00)
                self.mdb.models['Model-1'].steps['Remove'].adaptiveMeshDomains['Remove'].setValues(frequency=self.remeshingParameters[0], meshSweeps=self.remeshingParameters[1])
    ...
    def createPredefinedField(self,*args,**kwargs):
        ...
        if self.analysisType.startswith('Standard'):
            ...
            self.mdb.models['Model-1'].fieldOutputRequests['Contact_Results'].move('Indent', 'Pre-Stress')
            self.mdb.models['Model-1'].fieldOutputRequests['F-Output-1'].move('Indent', 'Pre-Stress')
            self.mdb.models['Model-1'].fieldOutputRequests['F-Output-2'].move('Indent', 'Pre-Stress')
            self.mdb.models['Model-1'].fieldOutputRequests['Plastic_Zone'].move('Indent', 'Pre-Stress')
            self.mdb.models['Model-1'].fieldOutputRequests['Reaction_Part_A'].move('Indent', 'Pre-Stress')
            self.mdb.models['Model-1'].fieldOutputRequests['Reaction_Part_B'].move('Indent', 'Pre-Stress')
            self.mdb.models['Model-1'].historyOutputRequests['H-Output-1'].move('Indent', 'Pre-Stress')
            ...
            self.mdb.models['Model-1'].boundaryConditions['TA_Top_Axial-Fixed_BC'].deactivate('Indent')
            self.mdb.models['Model-1'].boundaryConditions['TA_Bottom_Axial-Fixed_BC'].move('Initial', 'Indent')
    ...
    def createLoadsBCs(self,*args,**kwargs):
        ...
        if self.analysisType.startswith('Standard'):
            ...
            if self.controlType == 'Force':
                ...
                self.mdb.models['Model-1'].boundaryConditions['Indenter-Position_BC'].setValuesInStep(stepName='Indent', u2=FREED)
                self.mdb.models['Model-1'].boundaryConditions['Indenter-Position_BC'].setValuesInStep(stepName='Remove', u2=0.0)

                region = self.mdb.models['Model-1'].rootAssembly.sets['mSet-1']
                self.mdb.models['Model-1'].ConcentratedForce(name='Indenter_Force', createStepName='Indent', region=region, cf2=-self.indenterForce, amplitude='Std-Indent-%s'%(self.ampFunction), distributionType=UNIFORM, field='', localCsys=None)
                self.mdb.models['Model-1'].loads['Indenter_Force'].setValuesInStep(stepName='Remove', cf2=-self.indenterForce, amplitude='Std-Remove-%s'%(self.ampFunction))
                ...
                self.mdb.models['Model-1'].boundaryConditions['Indenter-Position_BC'].setValuesInStep(stepName='Indent', u2=-self.indenterDepth, amplitude='Std-Indent-%s'%(self.ampFunction))
                self.mdb.models['Model-1'].boundaryConditions['Indenter-Position_BC'].setValuesInStep(stepName='Remove', u2=-self.indenterDepth, amplitude='Std-Remove-%s'%(self.ampFunction))
                ...
                region = self.mdb.models['Model-1'].rootAssembly.sets['mSet-1']
                self.mdb.models['Model-1'].VelocityBC(name='Initial_Velocity', createStepName='Indent', region=region, v1=UNSET, v2=-self.indenterVelocity, vr3=UNSET, amplitude=UNSET, localCsys=None, distributionType=UNIFORM, fieldName='')
            ...
            if self.controlType == 'Force':
                ...
                region = self.mdb.models['Model-1'].rootAssembly.sets['mSet-1']
                self.mdb.models['Model-1'].ConcentratedForce(name='Indenter_Force', createStepName='Indent', region=region, cf2=-self.indenterForce, amplitude='Exp-Indent-%s'%(self.ampFunction), distributionType=UNIFORM, field='', localCsys=None)
                try:
                    self.mdb.models['Model-1'].loads['Indenter_Force'].setValuesInStep(stepName='Dwell-Loaded', cf2=-self.indenterForce)
                except:
                    pass
                self.mdb.models['Model-1'].loads['Indenter_Force'].setValuesInStep(stepName='Remove', cf2=-self.indenterForce, amplitude='Exp-Remove-%s'%(self.ampFunction))
                try:
                ...
                self.mdb.models['Model-1'].boundaryConditions['Indenter-Position_BC'].setValuesInStep(stepName='Indent', u2=-self.indenterDepth, amplitude='Exp-Indent-%s'%(self.ampFunction))
                try:
                    self.mdb.models['Model-1'].boundaryConditions['Indenter-Position_BC'].setValuesInStep(stepName='Dwell-Loaded', u2=0.0)
                except:
                    pass
                self.mdb.models['Model-1'].boundaryConditions['Indenter-Position_BC'].setValuesInStep(stepName='Remove', u2=-self.indenterDepth, amplitude='Exp-Remove-%s'%(self.ampFunction))
                try:
                ...
                self.mdb.models['Model-1'].VelocityBC(name='Initial_Velocity',
                    createStepName='Indent', region=region, v1=UNSET, v2=-self.indenterVelocity, vr3=UNSET, amplitude=UNSET, localCsys=None, distributionType=UNIFORM, fieldName='')
...
class General_Output(object):
    ...
    def ContactForceVector(self,*args,**kwargs):
        ...
        for key in self.odb.steps['Indent'].frames[-1].fieldOutputs.keys():

            if key.startswith('CNORMF'): keyCNORMF = key

            if key.startswith('CSHEARF'): keyCSHEARF = key

        sortedCNORMF = self.GeneralResults(sortedResults=True,sortDirection=1,stepName='Indent',instanceName='TEST_ARTICLE-1',setName='RESULTS_SURF_SET', resultName=keyCNORMF, writeCSV=False)

        sortedCSHEARF = self.GeneralResults(sortedResults=True,sortDirection=1,stepName='Indent',instanceName='TEST_ARTICLE-1',setName='RESULTS_SURF_SET', resultName=keyCSHEARF, writeCSV=False)
...
class Indentation_Output(General_Output):
    ...
    def VolumeDispCyl(self,*args,**kwargs):
        ...
        sortedNodeSet = self.GeneralResults(sortedResults=True,sortDirection=1,stepName='Remove',instanceName='TEST_ARTICLE-1',setName='RESULTS_SURF_SET', resultName='COORD', writeCSV=False)
    ...
    def NormalizerVariables(self,*args,**kwargs):
        ...
        #-----------------------------------------------------------------------
        sortedElementSet = self.GeneralResults(sortedResults=True,sortDirection=-1,stepName='Indent',instanceName='TEST_ARTICLE-1',setName='RESULTS_SURF_SET', resultName='PE', writeCSV=False)
        ...
        #-----------------------------------------------------------------------
        sortedElementSet = self.GeneralResults(sortedResults=True,sortDirection=2,stepName='Indent',instanceName='TEST_ARTICLE-1',setName='RESULTS_CL_SET', resultName='PE', writeCSV=False)
        ...
        #-----------------------------------------------------------------------
        sortedElementSet = self.GeneralResults(sortedResults=True,sortDirection=-1,stepName='Remove',instanceName='TEST_ARTICLE-1',setName='RESULTS_SURF_SET', resultName='PE', writeCSV=False)
        ...
        #-----------------------------------------------------------------------
        sortedElementSet = self.GeneralResults(sortedResults=True,sortDirection=2,stepName='Remove',instanceName='TEST_ARTICLE-1',setName='RESULTS_CL_SET', resultName='PE', writeCSV=False)
        ...
        #-----------------------------------------------------------------------
        sortedElementSet = self.GeneralResults(sortedResults=True,sortDirection=2,stepName='Indent',instanceName='TEST_ARTICLE-1',setName='RESULTS_CL_SET', resultName='COORD', writeCSV=False)

        # for value in sortedElementSet: print value

        normalVars['indentDepth_IndentY'] = sortedElementSet[-1][-1]
        #-----------------------------------------------------------------------
        sortedElementSet = self.GeneralResults(sortedResults=True,sortDirection=2,stepName='Remove',instanceName='TEST_ARTICLE-1',setName='RESULTS_CL_SET', resultName='COORD', writeCSV=False)
        ...
        #-----------------------------------------------------------------------
        sortedCoordSet = self.GeneralResults(sortedResults=True,sortDirection=2,stepName='Remove',instanceName='TEST_ARTICLE-1',setName='RESULTS_SURF_SET', resultName='COORD', writeCSV=False)
