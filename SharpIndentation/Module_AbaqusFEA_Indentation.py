...
class General_Analysis(object):
    ...
    def createElasticMaterials(self,*args,**kwargs):
        ...
        self.partIndType = kwargs.get('partIndType','Rigid' )
        self.matTaName = kwargs.get('matTaName','a-SiO2')
        self.matTaEYM = kwargs.get('matTaEYM',70.0e3)
        self.matTaEPR = kwargs.get('matTaEPR',0.15)
        self.matTaDensity = kwargs.get('matTaDensity',2.20e-9)
        self.matTaRayDamp = kwargs.get('matTaRayDamp',(0.0,0.0))

        #Indenter: Linear-Elastic
        if self.partIndType == 'Deformable':

            self.matIndName = kwargs.get('matIndName','Diamond')
            ...
            if self.matIndName == 'Diamond':
                ...
                if self.matIndName.endswith(' (Specify Below)'): self.matIndName = self.matIndName.rstrip(' (Specify Below)')

                self.matIndEYM = kwargs.get('matIndEYM',0.0)
                self.matIndEPR = kwargs.get('matIndEPR',0.0)
                self.matIndDensity = kwargs.get('matIndDensity',0.0)
        ...
        if self.anType.startswith('Explicit'): self.mdb.models['Model-1'].materials['Elastic_'+self.matTaName].Damping(alpha=self.matTaRayDamp[0], beta=self.matTaRayDamp[1])
    ...
    def createPlasticMaterials(self,*args,**kwargs):
        ...
        self.matTaPsModel = kwargs.get('matTaPsModel','vonMises')

        self.matTaPsVM = kwargs.get('matTaPsVM',7.50e3)

        self.matTaPsGTNrd = kwargs.get('matTaPsGTNrd',1.0)
        self.matTaPsGTNq = kwargs.get('matTaPsGTNq',(1.0,1.0,1.0))

        self.matTaPsKerm = kwargs.get('matTaPsKerm',(6500.0, -11500.0, 100000.0))

        self.matTaPsMoln = kwargs.get('matTaPsMoln',(5.0, 5700.0, 5000.0, -3000.0, -0.196, 3.0, 4.0, -20000.0, 7000.0))

        self.matTaPsDPCap = kwargs.get('matTaPsDPCap',(6500.0, 0.001, 1.75, 11500.0, 0.05, 1.0))
        self.matTaPsDPCapHard = kwargs.get('matTaPsDPCapHard',((11500.0, 0.0), (12500.0, 0.01)))

        # matPostYielding = kwargs.get('matPostYielding',[1.00,0.10])

        if self.matTaPsModel == 'vonMises':
        ...
        elif self.matTaPsModel == 'Gurson-Tvergaard-Needleman Porous Metal Plasticity' or self.matTaPsModel == 'GTN-pmp':

            self.matTaPsModel = 'GTN-pmp'
        ...
        elif self.matTaPsModel == 'Drucker-Prager Cap wHardening' or self.matTaPsModel == 'DP-Cap':
        ...
        elif self.matTaPsModel == 'Drucker-Prager-Cap (Molnar,2017)' or self.matTaPsModel == 'Moln2017':

            self.matTaPsModel = 'Moln2017'
        ...
        # # for self.matTaNameIter in  self.mdb.models['Model-1'].materials.keys():

            # # if hasattr(self.mdb.models['Model-1'].materials[self.matTaNameIter], 'plastic'):

                # # self.mdb.models['Model-1'].Material(name='PostYield_' + self.matTaNameIter,  objectToCopy=self.mdb.models['Model-1'].materials[self.matTaNameIter])

                # # self.mdb.models['Model-1'].materials['PostYield_' + self.matTaNameIter].Plastic(table=matPostYieldingTable)
    ...
    def createSteps(self,*args,**kwargs):
        ...
        self.anTimeStep = args[0]

        self.outpFieldInt = kwargs.get('outpFieldInt',100)
        self.outpHistInt = kwargs.get('outpHistInt',100)
        self.anBulkViscosity = kwargs.get('anBulkViscosity',[0.06,0.06,0.06,0.06])
        ...
        if self.anType.startswith('Standard'):
            ...
            if self.anJobName.endswith('_pre'):
                ...
                self.mdb.models['Model-1'].FieldOutputRequest(name='Reaction_Force_Combo_A', createStepName='Loading', variables=('U', ), numIntervals=self.outpHistInt, timeMarks=ON, region=regionDef1, sectionPoints=DEFAULT, rebar=EXCLUDE)
                self.mdb.models['Model-1'].FieldOutputRequest(name='Reaction_Force_Combo_B', createStepName='Loading', variables=('RF', ), numIntervals=self.outpHistInt, timeMarks=ON, region=regionDef2, sectionPoints=DEFAULT, rebar=EXCLUDE)
    ...
    def createPredefinedField(self,*args,**kwargs): #The standard solver may not work yet, top may rise...

        self.bcTaPreStress = args[0]
    ...
    def createJob(self,*args,**kwargs):
        ...
        if self.matTaPsModel == 'Kerm2008': self.mdb.jobs[self.jobname].setValues(userSubroutine='%s\\abaqus_plugins\\SharpIndentation\\kermouche2008.for' %(os.path.expanduser('~')))
        if self.matTaPsModel == 'Moln2017': self.mdb.jobs[self.jobname].setValues(userSubroutine='%s\\abaqus_plugins\\SharpIndentation\\molnar2017.for' %(os.path.expanduser('~')))
    ...
    def expectedDepthForce(self,*args,**kwargs):
        ...
        print('Predicting Indenter Depth for Force (%s) in Job (%s)...' %(self.indenterForce,self.anJobName))
        ...
        try: bcIndDepth = ((highValues[0]-lowValues[0])/(highValues[1]-lowValues[1])) * (self.indenterForce-lowValues[1]) + lowValues[0]
        ...
        print >> sys.__stdout__, ('Predicted Indenter Depth: %s um' %(bcIndDepth))
    ...
    def expectedDepthEnergy(self,*args,**kwargs): #This function is in-work, hasn't been fully tested yet.
        ...
        print('Predicting Indenter Depth for Energy (%s) in Job (%s)...' %(self.inputEnergy,self.anJobName))
        ...
        try: bcIndDepth = ((highValues[0]-lowValues[0])/(highValues[1]-lowValues[1])) * (self.inputEnergy-lowValues[1]) + lowValues[0]
        except: raise ValueError('The pre_upg_RF_Work.csv needs to have values at greater depth; redo pre-analysis with greater depth.')

        print('Predicted Indenter Depth: %s um' %(bcIndDepth))
...
class ASym_Analysis(General_Analysis):
    ...
    def createInteractions(self,*args,**kwargs):
        ...
        self.partIndMass = args[1]
        self.contFriciton = args[2]
        self.contType = args[3]
    ...
    def createLoadsBCs(self,*args,**kwargs):
        ...
        self.bcIndDepth = kwargs.get('bcIndDepth',0.0)
...
class ASym_Indenter_Analysis(ASym_Analysis):
        ...
        #Discrete Deformable Indenter
            ...
            if self.iRad == 0.0:
                ...
                if self.iFlat > 0.0:
                    ...
                    #--------------Add to other deformable indenters with correct Mask values--------------------
            ...
            # p.setMeshControls(regions=pickedRegions, elemShape=QUAD_DOMINATED, allowMapped=False)
            ...
            # p.DatumPlaneByPrincipalPlane(principalPlane=XZPLANE, offset=0.25 * self.articlePart[1]*self.taL)
            # pickedFaces = p.faces.getSequenceFromMask(mask=('[#1 ]', ), )
            # p.PartitionFaceByDatumPlane(datumPlane=p.datums[9], faces=pickedFaces)
    ...
    def createAssembly(self,*args,**kwargs):
        ...
        if self.indenterType == 'Rigid':
            ...
            a = self.mdb.models['Model-1'].rootAssembly
            a.translate(instanceList=('Indenter-1', ), vector=(0.0, self.bcIndOffset, 0.0))
...
class HSym_Indenter_Analysis(Sym_Analysis):
    ...
    def createHalfIndenter(self,*args,**kwargs):
        ...
        #Discrete Deformable Indenter
            ...
            # # if (self.matIndName is not None) and (not self.matIndName == 'None'):
...
class ASym_Pillar_Analysis(ASym_Analysis):
    ...
    def createPillarIndenter(self,*args,**kwargs):
        ...
        self.iAng = np.radians(kwargs.get('partIndDAngle',0.0))
        self.iRad = kwargs.get('partIndRadius',self.topDiameter/10.0)
        self.iFlat = kwargs.get('partIndFlat',self.topDiameter*1.5)
        indMeshMultiples = kwargs.get('indMeshMultiples',[1.0,10.0])
    ...
    def createAssembly(self,*args,**kwargs):
        ...
        self.bcIndOffset = kwargs.get('bcIndOffset',self.totalHeight)
